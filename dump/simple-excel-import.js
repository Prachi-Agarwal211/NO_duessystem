...
async function simpleExcelImport() {
  const excelFilePath = path.join(__dirname, '..', 'data', 'Student data JU Jan2026.xlsx');
  
  console.log('🎯 SIMPLE EXCEL IMPORT FOR NO DUES FORM');
  console.log('=====================================\n');
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        // Direct insert into student_data table
        const { data: result, error } = await supabaseAdmin
          .from('student_data')
          .insert({
            registration_no: enrollNo,
...
simpleExcelImport().catch(console.error);
