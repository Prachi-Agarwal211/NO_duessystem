...
// Generate simple student data (no alumni required)
function generateStudentData(enrollmentNo) {
  ...
  return {
    ...
    school_id: null, // Keep null for simplicity
    course_id: null,
    branch_id: null,
    ...
    alumni_screenshot_url: null, // No alumni requirement
    ...
    final_certificate_generated: true, // Certificate generated
...
async function createCompletedStudents() {
  try {
    console.log('🚀 Creating completed student records (no alumni requirement)...');
        ...
        try {
          ...
          if (existing) {
            ...
            if (error) {
              ...
              console.log(`✅ Created ${enrollmentNo} as completed with certificate`);
              successCount++;
              createdRecords.push(data[0]);
            }
          }
    ...
    console.log('📜 All students have been marked as completed with certificates generated!');
...
async function verifyCreation() {
  try {
    ...
    console.log('\n🎯 Your dashboards should now show all completed students!');
